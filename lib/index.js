  ...
  ctx.effect(() => {
    const disposers = [
      ctx.webServer.register({
        ...
        path: '/whale/state',
        handler: (_req, res) => {
          // The manifest's mtime fingerprints the asset table: clients refetch
          // it when this changes, so swapped-in pets appear without a reload.
          let v = 0
          try { v = fs.statSync(path.join(assetsDir, 'manifest.json')).mtimeMs } catch { /* missing manifest keeps v=0 */ }
          sendJson(res, 200, { ...machine.snapshot, v, t: Date.now() })
        },
      }),
      ctx.webServer.register({
        kind: 'exact',
