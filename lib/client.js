    ...
    factory: function (require) {
      ...
      function AquariumView({ manifest, snap, onExit }) {
        ...
        if (swim.current === null) {
          swim.current = {
            whale: { x: 140, y: 0.72, tx: 400, ty: 0.72, speed: 60, angle: 0, dir: 1 },
        ...
        React.useEffect(() => {
          ...
          const tick = (now) => {
            ...
            // Horizontal-biased wander in the lower band; states steer it.
            // Positions are viewport fractions, not pixels.
            if (state === 'glass-tap') { w.tx = Math.min(0.9, 1 - 170 / W); w.ty = 0.62 }
            else if (state === 'sleep' || state === 'nightcap') { w.ty = 0.9; w.tx = w.x }
            else if (Math.hypot((w.tx - w.x) * W, (w.ty - w.y) * H) < 40) {
              w.tx = 0.08 + Math.random() * 0.6
              w.ty = 0.55 + Math.random() * 0.35
            }
            w.x = Math.max(0.02, Math.min(0.92, w.x))
            w.y = Math.max(0.15, Math.min(0.92, w.y))
            const px = w.x * W, py = w.y * H
            const tx = w.tx * W, ty = w.ty * H
            const dx = tx - px, dy = ty - py
            const d = Math.hypot(dx, dy) || 1
            const vx = dx / d, vy = dy / d
            w.dir = vx >= -0.05 ? 1 : -1
            // Smoothed pitch (±24°) so vertical moves glide instead of sliding.
            const target = Math.max(-0.42, Math.min(0.42, Math.atan2(vy, Math.abs(vx))))
            w.angle += (target - w.angle) * Math.min(1, dt * 5)
            w.x += (vx * w.speed * dt) / W
            w.y += (vy * w.speed * dt) / H
            const el = refs.current[0]
            if (el) el.style.transform = 'translate(' + (w.x * W) + 'px,' + (w.y * H) + 'px) scaleX(' + w.dir + ') rotate(' + (w.dir * w.angle) + 'rad)'
            // Token fish school trails the whale.
            swim.current.fish.forEach((f, i) => {
              const fel = refs.current[1 + i]
              if (!fel) return
              const show = i < tools ? '1' : '0'
              if (fel.style.opacity !== show) fel.style.opacity = show
              if (show === '0') return
              f.phase += dt * 2
              const fx = w.x * W + f.ox + Math.sin(f.phase) * 10
              const fy = w.y * H + Math.sin(f.phase * 0.7 + i) * 14
              fel.style.transform = 'translate(' + fx + 'px,' + fy + 'px) scaleX(' + w.dir + ')'
            })
        ...
        return h('div', {
          ...
          h('div', {
            ...
            style: { position: 'absolute', left: 0, top: 0, width: '150px', willChange: 'transform' },
          },
            h('img', {
              key: sprite,
              src: '/whale/assets/' + sprite,
              style: { width: '100%', display: 'block', imageRendering: 'pixelated', filter: 'drop-shadow(0 6px 12px rgba(0,0,0,0.35))' },
              draggable: false, alt: '',
            })),
          pct !== null ? h('div', {
            style: {
