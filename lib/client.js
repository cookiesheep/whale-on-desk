  ...
  window.__ModuleLoader__.load({
    ...
    factory: function (require) {
      ...
      const STATE_LABELS = ZH ? {
        ...
        proud: '小骄傲', shy: '害羞', suspicious: '怀疑', excited: '兴奋', 'sad-puppy': '委屈',
        review: '审查代码', push: '推送成功', 'error-spiral': '连连报错', 'success-streak': '三连胜!',
        'token-fountain': '文思泉涌', 'chase-fish': '追小鱼', 'hide-and-seek': '躲猫猫',
        'bubble-ring': '吐圈圈', birthday: '生日快乐', shrimp: '加餐小虾',
      } : {
        ...
        proud: 'proud', shy: 'shy', suspicious: 'hmm?', excited: 'excited', 'sad-puppy': 'pleaded',
        review: 'reviewing', push: 'pushed!', 'error-spiral': 'error spiral', 'success-streak': 'streak!',
        'token-fountain': 'in flow', 'chase-fish': 'chasing', 'hide-and-seek': 'peekaboo',
        'bubble-ring': 'bubble ring', birthday: 'happy birthday', shrimp: 'shrimp!',
      }
      ...
      const bubbleText = (snap, state, quip) => quip
        ...
        || (snap.hint && snap.hint.kind === 'say' ? snap.hint.text : null)
        || (snap.hint && snap.hint.kind === 'context' ? (ZH ? '吃饱了' : 'Full: ') + snap.hint.pct + '%' : null)
        || (snap.hint && snap.hint.kind === 'levelup' ? (ZH ? '升级了! Lv.' : 'Level up! Lv.') + snap.hint.level + (ZH ? ' 🎉' : ' 🎉') : null)
        || (snap.hint && snap.hint.kind === 'title' ? (ZH ? '解锁称号「' : 'Title unlocked: "') + snap.hint.name + (ZH ? '」!' : '"!') : null)
      ...
      const FACES_RIGHT = new Set(['glass-tap.gif', 'swim-fast.gif', 'tool-run.gif', 'hide-and-seek.gif'])
      ...
      function AquariumView({ manifest, snap, scales, onExit }) {
        const pick = (...names) => {
        ...
        React.useEffect(() => {
          ...
          const tick = (now) => {
            ...
            if (bEl) {
              // The 320px cell has transparent headroom above the drawn body;
              // +12 lands the bubble just above the visible head.
              bEl.style.transform = 'translate(' + (w.x * W) + 'px,' + (w.y * H + 12 + bobY) + 'px) translateX(-50%)'
            }
        ...
        return h('div', {
          ...
          // Warm godrays through the water column.
          h('img', {
            src: '/whale/assets/godray-enhanced.png',
            style: {
              position: 'absolute', inset: 0, width: '100%', height: '100%',
              objectFit: 'cover', opacity: light ? '0.45' : '0.8', pointerEvents: 'none',
            },
            draggable: false, alt: '',
          }),
          // Water-surface shimmer caps the tank (top quarter only).
          h('img', {
            src: '/whale/assets/water-surface-' + (light ? 'light' : 'dark') + '.png',
            style: {
              position: 'absolute', left: 0, right: 0, top: 0, width: '100%',
              opacity: light ? '0.75' : '0.9', pointerEvents: 'none',
            },
            draggable: false, alt: '',
          }),
          [0, 1, 2].map((i) => h('div', {
          ...
          // Jellyfish drift in the mid-far water: slow vertical float,
          // dimmer = further away.
          [
            { src: 'jellyfish-large.gif', left: '14%', top: '16%', w: 120, dur: 11, op: 0.85 },
            { src: 'jellyfish-small.gif', left: '47%', top: '8%', w: 66, dur: 14, op: 0.6 },
            { src: 'jellyfish-small.gif', left: '78%', top: '24%', w: 52, dur: 17, op: 0.45 },
          ].map((j, i) => h('img', {
            key: 'jf' + i,
            src: '/whale/assets/' + j.src,
            className: 'whale-on-desk-drift',
            style: {
              position: 'absolute', left: j.left, top: j.top, width: j.w + 'px',
              animationDuration: j.dur + 's', animationDelay: (-i * 4) + 's',
              opacity: j.op, imageRendering: 'pixelated', pointerEvents: 'none',
              filter: light ? 'saturate(0.85)' : 'brightness(0.95)',
            },
            draggable: false, alt: '',
          })),
          // Far fish school: a dim band crossing behind everything.
          h('img', {
            src: '/whale/assets/fish-school.gif',
            className: 'whale-on-desk-school',
            style: {
              position: 'absolute', left: 0, top: '30%', width: '420px',
              opacity: light ? '0.5' : '0.55', imageRendering: 'pixelated', pointerEvents: 'none',
              filter: light ? 'saturate(0.6)' : 'brightness(0.8)',
            },
            draggable: false, alt: '',
          }),
          // Floor trinkets.
          h('img', { src: '/whale/assets/starfish.png', style: { position: 'absolute', left: '44%', bottom: '26px', width: '52px', imageRendering: 'pixelated', opacity: '0.85', pointerEvents: 'none' }, draggable: false, alt: '' }),
          h('img', { src: '/whale/assets/shell.png', style: { position: 'absolute', left: '71%', bottom: '30px', width: '44px', imageRendering: 'pixelated', opacity: '0.8', pointerEvents: 'none' }, draggable: false, alt: '' }),
          ...
          h('div', {
            ...
            h('img', {
              ...
              style: {
                ...
                transform: (scales && scales[state] ? 'translateY(' + (-scales[state].dyf * 100) + '%) scale(' + scales[state].s + ')' : '') + (spriteTransform(sprite) ? ' scaleX(-1)' : ''),
              },
              draggable: false, alt: '',
            })),
          aqBubble ? h('div', {
          ...
          // Foreground kelp: near-camera silhouettes framing the edges.
          h('img', {
            src: '/whale/assets/foreground-kelp.gif',
            style: {
              position: 'absolute', left: '-30px', bottom: '-60px', width: '300px',
              opacity: light ? '0.4' : '0.5', imageRendering: 'pixelated', pointerEvents: 'none',
              filter: 'blur(1px)',
            },
            draggable: false, alt: '',
          }),
          h('img', {
            src: '/whale/assets/foreground-kelp.gif',
            style: {
              position: 'absolute', right: '-40px', bottom: '-70px', width: '360px', transform: 'scaleX(-1)',
              opacity: light ? '0.4' : '0.5', imageRendering: 'pixelated', pointerEvents: 'none',
              filter: 'blur(1.5px)',
            },
            draggable: false, alt: '',
          ...
          h('style', null,
            ...
            '.whale-on-desk-drift{animation-name:whale-on-desk-drift;animation-timing-function:ease-in-out;animation-iteration-count:infinite;animation-direction:alternate}' +
            '@keyframes whale-on-desk-drift{from{transform:translate(0,0)}to{transform:translate(26px,-34px)}}' +
            '.whale-on-desk-school{animation:whale-on-desk-cross 46s linear infinite}' +
            '@keyframes whale-on-desk-cross{from{transform:translateX(-440px)}to{transform:translateX(100vw)}}' +
      ...
      function WhaleOverlay() {
        ...
        React.useEffect(() => {
          const timer = setInterval(() => {
            ...
            // Aquarium idle antics: occasionally chase a fish or blow a ring.
            if (aquarium && Math.random() < 0.35) {
              fetch('/whale/poke', { method: 'POST', body: JSON.stringify({ antics: Math.random() < 0.6 ? 'chase-fish' : 'bubble-ring' }) })
            }
          }, 90_000)
          return () => clearInterval(timer)
        }, [snap.state, menuOpen, aquarium])

        // Coming back to the tab startles a quick hide-and-seek.
        React.useEffect(() => {
          let lastAntic = 0
          const onVis = () => {
            if (document.visibilityState !== 'visible') return
            if (Date.now() - lastAntic < 10 * 60_000) return
            lastAntic = Date.now()
            fetch('/whale/poke', { method: 'POST', body: '{"antics":"hide-and-seek"}' })
          }
              ...
              (body) => {
                ...
                if (typeof body.t === 'number' && body.t > lastT) {
                  ...
                  if (body.v !== undefined && body.v !== manifestV) {
                    ...
                    fetch('/whale/assets/scale.json', { cache: 'no-store' }).then(
                      (r) => (r.ok ? r.json() : null),
                      () => null,
                    ).then((sc) => { if (alive) setScales(sc || null) })
                  }
                }
              },
              () => {
        ...
        const onPointerDown = (event) => {
          ...
          petTimer.current = setTimeout(() => {
            if (drag.current && !drag.current.moved) {
              ...
              say(PETTED_LINE)
              burstHearts()
              fetch('/whale/poke', { method: 'POST', body: '{"pet":true}' })
            }
          }, 800)
        }
        ...
        const openDesktop = () => {
          setMenuOpen(false)
          fetch('/whale/desktop', { method: 'POST', body: '{}' })
        }
        const openPalette = () => {
          setMenuOpen(false)
        ...
        if (gone) return null
        if (aquarium) {
          return h(AquariumView, { manifest, snap, scales, onExit: () => setAquarium(false) })
        }
        ...
        return h('div', {
          ...
          hover && (snap.stats || snap.growth) ? h('div', {
            style: {
            ...
            (snap.growth ? 'Lv.' + snap.growth.level + (snap.growth.latestTitle ? '「' + snap.growth.latestTitle + '」' : '') + ' · ' : '')
            + (snap.stats
              ? (STATE_LABELS[state] ?? state) + ' · ⏱ ' + fmtDur(snap.stats.secs) + ' · 🔧 ' + snap.stats.tools
              : (snap.growth ? (ZH ? 'XP ' + snap.growth.xp + ' / 下一级还差 ' + snap.growth.toNext : 'XP ' + snap.growth.xp + ' / ' + snap.growth.toNext + ' to next') : ''))) : null,
          menuOpen ? h('div', {
            style: {
            ...
            file
              ? h('img', {
                  ...
                  style: {
                    ...
                    transform: (scales && scales[state] ? 'translateY(' + (-scales[state].dyf * 100) + '%) scale(' + scales[state].s + ')' : '') + (spriteTransform(file) ? ' scaleX(-1)' : ''),
                  },
                  draggable: false, alt: '',
                })
