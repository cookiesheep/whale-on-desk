...
;(function () {
  ...
  window.__ModuleLoader__.load({
    ...
    factory: function (require) {
      ...
      /* Fullscreen ambient mode: the whales swim free instead of sitting
         in the corner. All sprites are the same companion GIFs. */
      function AquariumView({ manifest, onExit }) {
        const whales = React.useMemo(() => {
          const pick = (...names) => {
            for (const n of names) if (manifest && manifest[n]) return manifest[n]
            return 'idle.gif'
          }
          return [
            { size: 130, src: pick('swim-fast'), speed: 34 },
            { size: 96, src: pick('idle'), speed: 24 },
            { size: 66, src: pick('idle'), speed: 17 },
          ]
        }, [manifest])
        const refs = React.useRef([])
        const swim = React.useRef(whales.map((w) => ({
          ...w, x: 120 + Math.random() * 300, y: 100 + Math.random() * 200,
          tx: 300, ty: 180, dir: 1,
        })))
        React.useEffect(() => {
          let raf = 0
          ...
          const tick = (now) => {
            ...
            swim.current.forEach((s, i) => {
              ...
              if (d < 14) {
                s.tx = 70 + Math.random() * Math.max(60, window.innerWidth - 220)
                s.ty = 60 + Math.random() * Math.max(60, window.innerHeight - 240)
              } else {
                s.dir = dx >= 0 ? 1 : -1
                s.x += (dx / d) * s.speed * dt
                s.y += (dy / d) * s.speed * dt
              }
              el.style.transform = 'translate(' + s.x + 'px,' + s.y + 'px) scaleX(' + s.dir + ')'
            })
        ...
        return h('div', {
          style: {
            ...
            background: 'linear-gradient(180deg, #0D1B2E 0%, #0D1420 45%, #0A1120 100%)',
          ...
          [0, 1, 2].map((i) => h('div', {
            ...
            style: { left: (12 + i * 28) + '%', width: (120 + i * 60) + 'px', animationDelay: (-i * 3) + 's' },
          })),
          ...
          whales.map((w, i) => h('div', {
            key: 'w' + i,
            ref: (el) => { refs.current[i] = el },
            style: { position: 'absolute', left: 0, top: 0, width: w.size + 'px', willChange: 'transform' },
          },
            h('img', {
              src: '/whale/assets/' + w.src,
          ...
          h('div', {
            ...
            style: {
              ...
              color: '#8FA3C4', font: '15px/1 system-ui', userSelect: 'none',
              background: 'rgba(26,16,48,0.6)', borderRadius: '999px', padding: '8px 14px',
            },
          }, '✕ 退出 (Esc)'),
          h('div', {
            style: { position: 'absolute', bottom: '14px', width: '100%', textAlign: 'center', color: '#4A5E80', font: '12px system-ui', pointerEvents: 'none' },
          }, 'whale-on-desk · 水族馆'),
          h('style', null,
            '.whale-on-desk-ray{position:absolute;top:-10%;bottom:-10%;background:linear-gradient(172deg,transparent 10%,rgba(191,227,255,0.07) 45%,transparent 85%);transform:rotate(10deg);animation:whale-on-desk-sway 9s ease-in-out infinite alternate;pointer-events:none}' +
            '@keyframes whale-on-desk-sway{from{transform:rotate(8deg) translateX(-14px)}to{transform:rotate(13deg) translateX(18px)}}' +
            '.whale-on-desk-bub{position:absolute;bottom:-24px;border-radius:50%;border:1px solid rgba(191,227,255,0.35);background:rgba(191,227,255,0.08);animation-name:whale-on-desk-rise;animation-timing-function:linear;animation-iteration-count:infinite;pointer-events:none}' +
            '@keyframes whale-on-desk-rise{0%{transform:translateY(0);opacity:0}12%{opacity:0.55}100%{transform:translateY(-108vh);opacity:0}}',
          ),
        )
      }
      ...
      function WhaleOverlay() {
        ...
        if (gone) return null
        if (aquarium) {
          return h(AquariumView, { manifest, onExit: () => setAquarium(false) })
        }
